...
const createOne = async(subject, section, instructions, duration, text, link) => {
...
const updateOne = async(examId, subject, section, instructions, duration, text, link) => {
    const exam = await db.query('UPDATE exams SET subject = $1, section = $2, duration = $3, instructions = $4, text = $5, link = $6 WHERE id = $7 RETURNING *', [subject, section, duration, instructions, text, link, examId]);
    return exam.rows[0];
}
